...
class CRinGeNet(torch.nn.Module) :
    def __init__(self) :
        ...
        #    torch.nn.Linear(3,1000), torch.nn.ReLU(),
        #    torch.nn.Linear(1000,1000), torch.nn.ReLU()
            torch.nn.Linear(3,50), torch.nn.LeakyReLU(0.1),
            torch.nn.Linear(50,50), torch.nn.LeakyReLU(0.1)
        )
        ...
        #    torch.nn.Linear(1,1000), torch.nn.ReLU(),
        #    torch.nn.Linear(1000,1000), torch.nn.ReLU()
            torch.nn.Linear(1,50), torch.nn.LeakyReLU(0.1),
            torch.nn.Linear(50,50), torch.nn.LeakyReLU(0.1)
        )
        
        self._mlp = torch.nn.Sequential(
            #torch.nn.Linear(12, 1024), torch.nn.ReLU(),
            #torch.nn.Linear(1024, 1024), torch.nn.ReLU(),
           # torch.nn.Linear(4000, 14784), torch.nn.ReLU()
            torch.nn.Linear(200, 14784), torch.nn.LeakyReLU(0.1) 
        )


        self._upconvs = torch.nn.Sequential(
            torch.nn.ConvTranspose2d(64, 64, 4, 2),  torch.nn.LeakyReLU(0.1),  # 24 x 44 
            torch.nn.Conv2d(64, 64, 3), torch.nn.LeakyReLU(0.1),               # 22 x 42 
                                                                                 
            torch.nn.ConvTranspose2d(64, 32, 4, 2), torch.nn.LeakyReLU(0.1),   # 46 x 86 
            torch.nn.Conv2d(32, 32, 3),  torch.nn.LeakyReLU(0.1),              # 44 x 84 
                                                                                 
            torch.nn.ConvTranspose2d(32, 32, 4, 2), torch.nn.LeakyReLU(0.1),   # 90 x 170
            torch.nn.Conv2d(32, 3, 3)                                  # 88 x 168
        )

#        self._sigmoid = torch.nn.Sigmoid()
        self._relu = torch.nn.LeakyReLU(0.1)
    def forward(self, x) :
...
#blob.bceloss = torch.nn.SmoothL1Loss()
...
f = open("Training_data_30N_LeakyRelu.txt", "wb")
...
print('Epoch',epoch,int(epoch+0.5),'Ending @', time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()))
