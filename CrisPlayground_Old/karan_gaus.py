...
#            torch.nn.BatchNorm1d(3),
            torch.nn.Linear(3,50), torch.nn.ReLU(),
        ...
        self._mlp = torch.nn.Sequential(
            torch.nn.Linear(500, 1024), torch.nn.ReLU(),
...
while epoch < TRAIN_EPOCH :
    ...
    for i,data in enumerate(train_loader) :
        ...
        if i == 0 or (i+1)%10 == 0:
            data = np.column_stack((epoch, res['loss']))
            np.savetxt(f, data)
